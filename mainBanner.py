...
def save_article_as_html(article, output_dir='articlesBanner', images_dir='imagesBanner'):
...
# Main function
def main():
    urls = read_urls('urlsBanner.txt')
    for url in urls:
        article = scrape_article(url)
        if article:
            html_file_path = save_article_as_html(article)
            upload_to_ftp('articlesBanner', 'articlesBanner')
            upload_images_to_ftp('imagesBanner', 'imagesBanner')
